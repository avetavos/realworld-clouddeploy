...
- A `live/<cloud>/platform` unit that generates the `helm`/`kubernetes` providers from the `cluster` outputs — the same pattern you used for [ShopMicro](/clouddeploy/en/deploy-shopmicro/helm-release/). Later modules (Keycloak, GrowthBook) add more `.tf` files to this same `modules/platform/` module and more inputs to this same unit — Datadog is the first tenant, not a one-off.
...
### 2. `modules/platform/` (secret + release)
...
### 3. `live/aws/platform/terragrunt.hcl` (wire it up)
