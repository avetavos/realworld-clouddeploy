...
An ordered dashboard with the golden signals for the gateway. A `template_variable` lets you filter the whole board by cluster, so one dashboard shows any cloud or all of them. `defaults` (a list) is the current attribute — the older single-value `default` is deprecated.
