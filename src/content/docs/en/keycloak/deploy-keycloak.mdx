...
The realm and OIDC client, via the `keycloak` provider. Every current release — 26.x today — ships the Quarkus distribution, which dropped the old `/auth` path prefix back in Keycloak 17, so issuer URLs are `https://<host>/realms/<realm>` with no `/auth` segment. The `keycloak` provider needs its own entry in `modules/platform/versions.tf` — it isn't a `hashicorp/*` provider, so Terraform can't resolve it without an explicit `source`.

```hcl
# modules/platform/versions.tf (addition)
terraform {
  required_providers {
...
inputs = {
  ...
  ingress_class           = "alb" # AWS Load Balancer Controller — see the ingress lesson
...
`gcp/` and `azure/` are the same file with their own hostnames and `ingress_class` (`gce`, `azure-application-gateway`) — the same native-per-cloud ingress classes [ShopMicro already uses](/clouddeploy/en/deploy-shopmicro/ingress-and-config/), not ingress-nginx.
...
4. The Quarkus distribution that every current Keycloak (26.x) ships removed a path segment that older OIDC guides still show. What is it, and where would leaving it in break discovery?
