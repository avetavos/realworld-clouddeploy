...
The same `variables.tf` and `node_size` map (mapped to GCP machine types), a VPC-native cluster referencing the network's secondary ranges by name (`pods`, `services`), and a separately managed node pool. It also turns on `workload_identity_config` — the [GCP IAM module](/clouddeploy/en/iam/workload-identity/) binds to it in a later module, so it has to be enabled here, on the cluster.
...
GKE defaults `deletion_protection` to true when it's unset, which would block Module 13's `terragrunt run --all destroy` — set it `false` explicitly, the same call already made for the [managed Postgres](/clouddeploy/en/data/managed-postgres/) instance. GKE's endpoint also comes back without a scheme and its CA is under `master_auth`. `outputs.tf` normalizes to the interface:
...
AKS requires an inline `default_node_pool` and a managed identity. The cluster lands in the resource group derived from the network. `node_size` maps to Azure VM sizes. `oidc_issuer_enabled` and `workload_identity_enabled` turn on the cluster's OIDC issuer — the [Azure IAM module](/clouddeploy/en/iam/workload-identity/) federates a managed identity against it in a later module.
