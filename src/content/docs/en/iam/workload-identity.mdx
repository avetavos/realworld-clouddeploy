...
Create a user-assigned identity, federate it to the cluster's OIDC issuer for one service account, and assign the custom role over one resource group. `azurerm_federated_identity_credential` takes the identity by `user_assigned_identity_id` — a full resource id — rather than a separate `resource_group_name` plus `parent_id`, in current azurerm.
