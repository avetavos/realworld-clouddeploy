...
The azurerm v5 provider requires a `features {}` block and a subscription. The provider reads `ARM_SUBSCRIPTION_ID` from the environment, so you don't hardcode it:
