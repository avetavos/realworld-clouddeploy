...
**เสียบเข้าตรงไหน:** CI plan step PR workflow ของคุณ produce Terraform plan อยู่แล้ว; ป้อน plan JSON นั้นให้ [Open Policy Agent](https://www.openpolicyagent.org/) (ผ่าน `conftest`) หรือบน HCP Terraform ให้ Sentinel แล้ว fail check เมื่อ policy ถูกละเมิด นี่เป็น gate ใหม่บน pipeline เดิม ไม่ใช่ pipeline ใหม่
