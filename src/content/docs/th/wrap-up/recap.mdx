---
title: Recap
description: "สิ่งที่คุณสร้าง, diagram ที่ผูกทุกอย่างเข้าด้วยกัน, และการตัดสินใจ multi-cloud ที่คุณ defend ได้แล้ว"
sidebar:
  order: 1
---
...
- **platform layer** — Datadog, Keycloak, และ GrowthBook — deploy โดย Helm และเหมือนกันไม่ว่าจะลงบน cloud ไหน
...
ประเด็นไม่เคยเป็นตัวใดตัวหนึ่งแบบโดด ๆ แต่คือการทำให้ทุกชิ้นประกอบกันได้เพื่อให้ "the platform" เป็นสิ่งที่คุณ `apply` ไม่ใช่ runbook ที่คุณทำตามด้วยมือ
...
อ่านจากบนลงล่าง: **module** อธิบาย *ว่า* แต่ละชิ้นของ infrastructure คืออะไร ครั้งเดียว **Terragrunt `live/`** จัดหา *ที่ไหนและด้วย input อะไร* ต่อ cloud สี่ infrastructure unit apply ในลำดับ dependency — `network` ก่อน `cluster`, `cluster` และ `network` ก่อน `data`, และ `iam` bind service account เข้ากับ cluster ด้านบนคือ **platform layer** ที่ไม่สนว่าอยู่บน cloud ไหน และจากนั้น **ShopMicro** เป็น workload **CI/CD** ห่อรอบทั้งหมด: ขับ `live/` tree และ Helm deploy เพื่อให้ repository ไม่ใช่ laptop ของใครคนหนึ่ง เป็น source of truth

## การตัดสินใจที่คุณอธิบายเหตุผลได้แล้ว

คุณค่าของ capstone ไม่ใช่ artifact — แต่คือการที่คุณตัดสินใจ architectural จริง ๆ และตอนนี้ argue สนับสนุนได้ รวม trade-off ด้วย:
...
| Cloud-neutral platform layer (Helm) | observability, identity, และ flag เรียนครั้งเดียวและรันเหมือนกันบน EKS, GKE, หรือ AKS | คุณต้องรันและ patch เองแทนที่จะใช้ managed equivalent ของแต่ละ cloud | [The Datadog agent →](/clouddeploy/th/datadog/the-datadog-agent/) |
...
ถ้ามีคนถาม "ทำไมสาม cloud?" หรือ "ทำไม Datadog อยู่ใน Helm ไม่ใช่ managed service?" คุณมีคำตอบที่เรียกชื่อ cost ไม่ใช่แค่ประโยชน์ นั่นคือความต่างระหว่างการ copy pattern กับการเป็นเจ้าของตัวเองจริง
...
- **หนึ่ง environment ต่อ cloud** ไม่มีการแยก `dev` / `staging` / `prod` — แค่ "the platform" ทีมจริงจะเพิ่ม environment; `live/` tree คือที่ที่จะไปลงพอดี
...
ไม่มีข้อไหนเป็นช่องว่างที่ต้องขอโทษ นี่คือเส้นแบ่งระหว่าง "เรียน pattern" กับ "operate ที่ scale" — และ [next lesson](/clouddeploy/th/wrap-up/next-steps/) คือเรื่องการข้ามเส้นนั้นอย่างจงใจ

## สรุป

ตอนนี้คุณ provision, observe, secure, และ tear down multi-cloud platform ทั้งหมดเป็น code ได้ — และที่สำคัญกว่า defend รูปทรงที่ออกมาได้ คุณรู้ว่าทำไม module กับ `live/` tree ถึงแยกกัน, ทำไม platform layer ถึง cloud-neutral, และการรันสาม cluster cost เท่าไหร่จริง ๆ

บทนี้ยังปิด **six-project Real-World Projects series** ด้วย แต่ละ guide rebuild แอปพลิเคชันจริงคนละตัวจาก repo ว่างเปล่า และ CloudDeploy คือที่ที่หนึ่งในนั้น — ShopMicro — ในที่สุดก็รันในโลกจริง ถ้าคุณยังไม่ได้ทำตัวอื่น ทั้งหมดรันได้จริง:
