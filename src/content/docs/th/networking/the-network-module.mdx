...
Terragrunt unit ที่ instantiate module unit นี้ inherit S3 backend และ `aws` provider จาก `root.hcl` (สร้างไว้ตั้งแต่ [Terragrunt & Remote State](/clouddeploy/th/terragrunt/)) ดังนั้นทั้งหมดที่ต้องจัดหาคือ source และ interface input สองตัว
...
list ที่ไม่ว่างสองตัวและ VPC id หนึ่งตัว — นั่นคือ contract ที่ถูกทำตาม [cluster module](/clouddeploy/th/kubernetes/) จะ consume `network_id` และ `private_subnet_ids` โดยไม่เคยรู้เลยว่ามาจาก AWS
