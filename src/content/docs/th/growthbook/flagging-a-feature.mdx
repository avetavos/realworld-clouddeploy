...
[GrowthBook รันอยู่](/clouddeploy/th/growthbook/deploy-growthbook/) และคุณมี SDK connection key แล้ว ตอนนี้เราจะเอามาใช้จริง: gate feature จริงของ ShopMicro ไว้หลัง flag แล้วพิสูจน์ว่าคุณพลิกจาก UI ได้โดยไม่แตะ deployment
...
เมื่อ flag ปิด `Recommendations` ว่างและ panel ไม่ render เมื่อเปิด storefront จะแสดง recommended products ไม่มี code path อื่นเปลี่ยนเลย
...
unit `live/<cloud>/shopmicro/terragrunt.hcl` ส่งค่าพวกนี้ผ่านไปต่อ cloud — API host เป็น address ของ Service จึงเหมือนกันบนทุก cluster
...
panel โผล่ขึ้นมาเพราะ SDK ของ pod ที่รันอยู่ได้รับการเปลี่ยนแปลงผ่าน SSE และ `EvalFeature(...).On` พลิกเป็น `true` toggle กลับไปปิดใน UI แล้ว count กลับไปเป็น `0` การพลิกแบบ live นั้นโดยที่ container ไม่ถูกแตะ คือผลตอบแทนทั้งหมด
...
1. ทำไม `flags.New` ถึงเรียก `EnsureLoaded` ตอน startup? request จะเห็นอะไรถ้า evaluate flag ก่อนที่ call นั้น return?
2. ด้วย SSE data source อะไรเกิดขึ้นจริง ๆ บน pod ที่รันอยู่เมื่อคุณ toggle flag ใน UI — และทำไมถึงไม่ต้อง redeploy?
3. `Attributes` บน child client มีไว้ทำอะไร ในเมื่อตอนนี้ flag เป็นแค่ on/off ง่าย ๆ? เปิดทางให้อะไรได้ในภายหลัง?
