...
dashboard ที่เรียงลำดับพร้อม golden signal สำหรับ gateway `template_variable` ให้คุณ filter ทั้งบอร์ดตาม cluster ดังนั้น dashboard เดียวแสดง cloud ใด ๆ หรือทั้งหมด `defaults` (เป็น list) คือ attribute ปัจจุบัน — `default` แบบค่าเดียวเก่านั้น deprecated แล้ว
