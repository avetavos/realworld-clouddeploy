...
[cluster module](/clouddeploy/th/kubernetes/) ให้ที่รัน pod แก่เรา ก่อนที่ pod พวกนั้นจะแตะ cloud API ได้ — อ่าน DNS zone, ดึง secret, เข้าถึง database — ต้องมีบางอย่างบอกว่า *identity นี้ทำได้เฉพาะสิ่งเหล่านี้เท่านั้นและไม่มากกว่านั้น* "บางอย่าง" นั้นคือ cloud IAM และหน้าตาต่างกันบนทุก cloud
...
policy พวกนี้เป็น inert จนกว่าจะมีอะไรมา assume โดยไม่มี static key ต่อไป [Workload identity →](/clouddeploy/th/iam/workload-identity/) สร้าง `iam` module ที่ยื่น identity ตัวหนึ่งเหล่านี้ให้ pod — IRSA บน AWS, Workload Identity บน GCP, Managed Identity บน Azure — โดยไม่มี long-lived credential อยู่ที่ไหนเลย
