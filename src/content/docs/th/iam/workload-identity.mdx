...
Look up OIDC issuer ของ cluster ด้วยชื่อ, register เป็น IAM OIDC provider, และเขียน assume-role policy ที่ condition `sub` ปักหมุด service account เดียวเป๊ะ
...
สร้าง Google service account, ให้สิทธิ์ Kubernetes service account impersonate ผ่าน binding `roles/iam.workloadIdentityUser`, และ attach custom role
...
สร้าง user-assigned identity, federate เข้ากับ OIDC issuer ของ cluster สำหรับ service account เดียว, และ assign custom role เหนือ resource group เดียว `azurerm_federated_identity_credential` รับ identity ผ่าน `user_assigned_identity_id` — resource id เต็ม — แทนที่จะเป็น `resource_group_name` บวก `parent_id` แยกกันแบบเดิม ใน azurerm เวอร์ชันปัจจุบัน
...
implementation ของ Azure ต้องมี input เพิ่มหนึ่งตัว `resource_group_name` เพื่อหา cluster — ค่านี้ default เป็นชื่อ platform ดังนั้น `live/` unit ยังส่งแค่ interface input สามตัว service account ของ pod พก annotation `azure.workload.identity/client-id` ตั้งค่าเป็น client id นี้ (และ pod template พก label `azure.workload.identity/use: "true"`)
...
1. cloud เช็คอะไรกันแน่ตอนที่ pod นำเสนอ token และทำไมนั่นถึงทำให้ static key ไม่จำเป็น?
2. ใน IRSA assume-role policy condition `:sub` ปักหมุดอะไร และอะไรพังถ้าค่านั้นผิดไปนิดเดียว?
